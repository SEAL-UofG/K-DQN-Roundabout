...
class AbstractLane(object):
    ...
    def on_lane(self,position: np.ndarray,longitudinal: float = None,lateral: float = None,
        margin: float = 0,) -> bool:
        """
        ...
        is_on = (np.abs(lateral) <= self.width_at(longitudinal) / 2 + margin
    ...
    def is_reachable_from(self, position: np.ndarray) -> bool:
        ...
        if self.forbidden:
            return False
        longitudinal, lateral = self.local_coordinates(position)
        is_close = (np.abs(lateral) <= 2 * self.width_at(longitudinal)
...
class SineLane(StraightLane):
    ...
    def position(self, longitudinal: float, lateral: float) -> np.ndarray:
        return super().position(longitudinal,lateral
