...
class RoadNetwork(object):
    ...
    def shortest_path1(self, start: str, goal: str) -> List[str]:
        """
...
class Road(object):
    ...
    def get_vehicle_count(self, lane_index: int) -> int:
        """ Return the number of vehicles in the specified lane. """
        num = 0  # 初始化计数器
        # print("车数量",len(self.vehicles))
        for i in self.vehicles:
            # print(i.lane_index[2])
            if i.lane_index[2] == lane_index:
                num += 1
        # print(num)
        return num

    def get_nearest_vehicle_lane(self, position: np.ndarray) -> int:
        """ Return the lane index of the nearest vehicle to the given position. """
        ...
        return nearest_vehicle.lane_index if nearest_vehicle else -1  # Return -1 if no vehicles are found
    ...
    def neighbour_vehicles(
        ...
        for v in self.vehicles + self.objects:
            if v is not vehicle:  # self.network.is_connected_road(v.lane_index,
    ...
    def surrounding_vehicles(self, vehicle: 'kinematics.Vehicle', lane_index: LaneIndex = None) \
            -> Tuple[Optional['kinematics.Vehicle'], Optional['kinematics.Vehicle']]:
        """
        Find the preceding and following vehicles of a given vehicle in a roundabout.
        ...
        v_fr = v_rr = v_fl = v_rl = None

        # mynote 车在环岛上
        lane_vehicles: Dict[LaneIndex, 'kinematics.Vehicle'] = {}
        for v in self.vehicles:
            lane_vehicles[v.lane_index] = v

        # Define the lane indices for the roundabout
        roundabout_lanes = [
        ...
        # print("sourround==",vehicle)
        if vehicle.lane_index in roundabout_lanes:
            for v in self.vehicles:
                if v is not None and v is not vehicle:
                    # 正前车
                    if v.lane_index[0] == vehicle.lane_index[1] and v.lane_index[2] == vehicle.lane_index[2]:
                        v_fr = v
                    # 正后车
                    elif v.lane_index[1] == vehicle.lane_index[0] and v.lane_index[2] == vehicle.lane_index[2]:
                        v_rr = v
                    #         旁车
                    elif v.lane_index[1] == vehicle.lane_index[1] and v.lane_index[2] == 1 - vehicle.lane_index[2]:
                        v_fl = v
                    #             旁后车
                    elif v.lane_index[1] == vehicle.lane_index[0] and v.lane_index[2] == 1 - vehicle.lane_index[2]:
                        v_rl = v

            # current_index = roundabout_lanes.index(vehicle.lane_index)#当索引为11 ("wx", "we", 0)，我们需要的("wx", "we", 1)索引为27
            # is_inner_lane = vehicle.lane_index[2] == 0
            # lane_count = len(roundabout_lanes) // 2  # 16
            #
            #
            #
            # prev_index = roundabout_lanes[(current_index - 1) % lane_count]#("wx1", "wx", 0)
            # next_index = roundabout_lanes[(current_index + 1) % lane_count]#("we", "we1", 0)
            #
            # if is_inner_lane:
            #     prev_outer_index = roundabout_lanes[current_index  + lane_count - 1] #11 +  16 -1 26 ("wx1", "wx", 1),//
            #     next_outer_index = roundabout_lanes[current_index + lane_count]# 11+16 ("wx", "we", 1)
            # else:
            #     #("wx", "we", 1) 索引 27 -16 11
            #     prev_outer_index = roundabout_lanes[current_index  - lane_count - 1]
            #     next_outer_index = roundabout_lanes[current_index - lane_count ]
            #
            # v_rr = lane_vehicles.get(prev_index)
            # v_fr = lane_vehicles.get(next_index)
            # v_rl = lane_vehicles.get(prev_outer_index)
            # v_fl = lane_vehicles.get(next_outer_index)



        # mynote 在汇入口
        elif (vehicle.lane_index == ("ses", "se", 0) or vehicle.lane_index == ("ses", "se", 1) or
              vehicle.lane_index == ("ees", "ee", 0) or vehicle.lane_index == ("ees", "ee", 1) or
              vehicle.lane_index == ("nes", "ne", 0) or vehicle.lane_index == ("nes", "ne", 1) or
              vehicle.lane_index == ("wes", "we", 0) or vehicle.lane_index == ("wes", "we", 1)):

            if (vehicle.lane_index == ("ses", "se", 0)):
                for v in self.vehicles:
                    if v is not vehicle:
                        if v.lane_index == ("se", "se1", 0):
                            v_fr = v
                        if v.lane_index == ("sx", "se", 0):
                            v_rr = v
                    v_fl = None
                    v_rl = None

            elif vehicle.lane_index == ("ses1", "se1", 0):
                for v in self.vehicles:
                    if v is not vehicle:
                        if v.lane_index == ("se1", "ex1", 0):
                            v_fr = v
                        if v.lane_index == ("se", "se1", 0):
                            v_rr = v
                    v_fl = None
            ...
            elif vehicle.lane_index == ("ees", "ee", 0):
                for v in self.vehicles:
                    if v is not vehicle:
                        if v.lane_index == ("ee", "ee1", 0):
                            v_fr = v
                        if v.lane_index == ("ex", "ee", 0):
                            v_rr = v
                    v_fl = None
                    v_rl = None

            elif vehicle.lane_index == ("ees1", "ee1", 0):
                for v in self.vehicles:
                    if v is not vehicle:
                        if v.lane_index == ("ee1", "nx1", 0):
                            v_fr = v
                        if v.lane_index == ("ee", "ee1", 0):
                            v_rr = v
                    v_fl = None
            ...
            elif vehicle.lane_index == ("nes", "ne", 0):
                for v in self.vehicles:
                    if v is not vehicle:
                        if v.lane_index == ("ne", "ne1", 0):
                            v_fr = v
                        if v.lane_index == ("nx", "ne", 0):
                            v_rr = v
                    v_fl = None
                    v_rl = None

            elif vehicle.lane_index == ("nes1", "ne1", 0):
                for v in self.vehicles:
                    if v is not vehicle:
                        if v.lane_index == ("ne1", "wx1", 0):
                            v_fr = v
                        if v.lane_index == ("ne", "ne1", 0):
                            v_rr = v
                    v_fl = None
            ...
            elif vehicle.lane_index == ("wes", "we", 0):
                for v in self.vehicles:
                    if v is not vehicle:
                        if v.lane_index == ("we", "we1", 0):
                            v_fr = v
                        if v.lane_index == ("wx", "we", 0):
                            v_rr = v
                    v_fl = None
                    v_rl = None

            elif vehicle.lane_index == ("wes1", "we1", 0):
                for v in self.vehicles:
                    if v is not vehicle:
                        if v.lane_index == ("we1", "sx1", 0):
                            v_fr = v
                        if v.lane_index == ("we", "we1", 0):
                            v_rr = v
                    v_fl = None
        ...
        return v_fr, v_rr,v_fl,v_rl



    def clear_vehicle(self):
        self.vehicles = [vehicle for vehicle in self.vehicles if hasattr(vehicle, "DEFAULT_TARGET_SPEEDS") or
                         (not vehicle.crashed and not hasattr(vehicle, "DEFAULT_TARGET_SPEEDS"))]
