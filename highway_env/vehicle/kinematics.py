...
class Vehicle(RoadObject):
    ...
    def __init__(
        ...
        self.TTC = float('inf')
        self.destination_name = {}  # 初始化空字典来存储目的地信息
    ...
    def compute_ttc(self,vehicle, other):
        """
        ...
        # 计算两车的相对速度
        relative_velocity = vehicle.speed - other.speed

        # 如果两车正在靠近,计算 TTC
        if relative_velocity > 0:
            ttc = distance / relative_velocity
        else:
            ttc = float('inf')  # 如果两车没有靠近,则 TTC 为无穷大
        self.TTC = ttc
        return ttc
    ...
    def sub_step(self, dt: float, acceleration, steering) -> None:
        ...
        self.position += ca.vertcat(vx * dt, vy * dt)
        # self.position += v * dt
        self.heading += self.speed * np.sin(beta) / (self.LENGTH / 2) * dt
    ...
    def destination(self) -> np.ndarray:
        if getattr(self, "route", None):
            ...
            # print("last_lane_index==",last_lane_index)
