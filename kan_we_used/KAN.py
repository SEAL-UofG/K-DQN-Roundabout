...
class KAN(nn.Module):
    ...
    def __init__(self, width=None, grid=3, k=3, noise_scale=0.1, noise_scale_base=0.1, base_fun=torch.nn.SiLU(),
        ...
        for l in range(self.depth):
            ...
            # bias
            bias = nn.Linear(width[l + 1], 1, bias=False).requires_grad_(bias_trainable).to(device)  # 添加.to(device)
            bias.weight.data *= 0.
        ...
        self.symbolic_fun = []
        for l in range(self.depth):
            sb_batch = Symbolic_KANLayer(in_dim=width[l], out_dim=width[l + 1]).to(device)  # 添加.to(device)
