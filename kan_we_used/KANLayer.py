...
class KANLayer(nn.Module):
    ...
    def __init__(self, in_dim=3, out_dim=2, num=5, k=3, noise_scale=0.1, scale_base=1.0, scale_sp=1.0,
        ...
        self.grid = torch.einsum('i,j->ij', torch.ones(size, ),
                                 torch.linspace(grid_range[0], grid_range[1], steps=num + 1)).to(
            device)  # 添加.to(device)
        self.grid = torch.nn.Parameter(self.grid).requires_grad_(False)
        noises = (torch.rand(size, self.grid.shape[1]) - 1 / 2) * noise_scale / num
        noises = noises.to(device)
        # shape: (size, coef)
        self.coef = torch.nn.Parameter(curve2coef(self.grid, noises, self.grid, k)).to(device)  # 添加.to(device)
        if isinstance(scale_base, float):
            self.scale_base = torch.nn.Parameter(torch.ones(size, ) * scale_base).requires_grad_(sb_trainable).to(
                device)  # 添加.to(device)
        else:
            self.scale_base = torch.nn.Parameter(scale_base).requires_grad_(sb_trainable).to(device)  # 添加.to(device)
        self.scale_sp = torch.nn.Parameter(torch.ones(size, ) * scale_sp).requires_grad_(sp_trainable).to(
            device)  # 添加.to(device)
        self.base_fun = base_fun

        self.mask = torch.nn.Parameter(torch.ones(size, )).requires_grad_(False).to(device)  # 添加.to(device)
        self.grid_eps = grid_eps
        self.weight_sharing = torch.arange(size).to(device)  # 添加.to(device)
        self.lock_counter = 0
        self.lock_id = torch.zeros(size).to(device)  # 添加.to(device)
        self.device = device
    ...
    def forward(self, x):
        ...
        # print("x after einsum shape:", x.shape)

        # preacts = x.permute(1,0).clone().reshape(batch, self.out_dim, self.in_dim)

        x = x.reshape(batch, self.size).permute(1, 0)
        # print("x after reshape and permute shape:", x.shape)

        # 继续打印其他变量的形状
        # print(f"Input shape: {x.shape}")
        # print(f"Expected output shape: (batch={batch}, size={self.size})")
        preacts = x.permute(1, 0).clone().reshape(batch, self.out_dim, self.in_dim)
        # print("preacts shape:", preacts.shape)
        # print(f"Output shape after einsum: {x.shape}")
        ...
        # print("y before permute shape:", y.shape)
        y = y.permute(1,0) # shape (batch, size)
        # print("y after permute shape:", y.shape)
        ...
        # print("y after scale and add shape:", y.shape)
        y = self.mask[None,:] * y
